...
def require_pkgresources(name):
    try:
        ...
        raise RuntimeError("'{0***REMOVED***' needs pkg_resources (part of setuptools).".format(name))
...
def sign(wheelfile, replace=False, get_keyring=get_keyring):
    ...
    payload = {"hash":"sha256=" + native(urlsafe_b64encode(hashlib.sha256(record_data).digest()))***REMOVED***
    ...
    for req in requirements:
        if req.endswith('.whl'):
            ...
            if os.path.exists(req):
                ...
                raise WheelError("No such wheel file: {***REMOVED***".format(req))
            continue
        ...
        if not matches:
            raise WheelError("No match for requirement {***REMOVED***".format(req))
...
def convert(installers, dest_dir, verbose):
    ...
    for pat in installers:
        for installer in iglob(pat):
            ...
            if verbose:
                sys.stdout.write("{0***REMOVED***... ".format(installer))
                sys.stdout.flush()
