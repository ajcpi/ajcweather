...
class WheelFile(object):
    ...
    def __lt__(self, other):
        if self.context != other.context:
            raise TypeError("{0***REMOVED***.context != {1***REMOVED***.context".format(self, other))
    ...
    def install(self, force=False, overrides={***REMOVED***):
        """
        ...
        for info in self.zipfile.infolist():
            ...
            if sep and basedir == self.datadir_name:
                ...
                if not sep:
                    raise ValueError("Invalid filename in wheel: {0***REMOVED***".format(name))
                target = get_path(key)
            else:
        ...
        if not force:
            for info, v in name_trans.items():
                ...
                if os.path.exists(dest):
                    raise ValueError("Wheel file {0***REMOVED*** would overwrite {1***REMOVED***. Use force if this is intended".format(k, dest))
